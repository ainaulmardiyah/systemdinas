...
class Kuisoner extends CI_Controller {
	...
	public function nextquestion()
	{
		...
		$id_staf = $this->session->iduserlogin;	
		$entropivalidasi = $this->entropy->validasiquestionentropy($gets, $id_staf);
	...
	if(count($validasikuisoner->result()) >= 1){
		echo "<script>
	...
	if(isset($_POST['NextQuestion'])) {
		...
		//if(count($validasikuisoner->row()) == NULL){
		$insertkuis = $this->answer->add($param);
	//	}
		;
...
$kp=0;
		$mno=0;
		foreach ($this->kuisoner->get()->result_array() as $row){
			$datapertanyaaan[] = (int) $row['id_pertanyaan'];
			//$data_kategori[] = (string) $row['aspek_subject'];
			foreach ($this->kuisoner->cekredudan($id_staf, $datapertanyaaan[$kp])->result_array() as $row){
				$dataredudan[$kp] = (int) $row['jumla_redudan'];
				foreach ($this->kuisoner->cekdataid($id_staf, $datapertanyaaan[$kp])->result_array() as $row){
					//$dataredudan[$kp] = (int) $row['jumla_redudan'];
		...
		$pss=0;
		for($llii=0; $llii<count($this->kuisoner->get()->result_array()); $llii++){

		
		foreach ($this->kuisoner->cekdataid( $id_staf, $datapertanyaaan[$llii])->result_array() as $row){
			//$dataredudan[$kp] = (int) $row['jumla_redudan'];
		
		$datagetquestion[$llii][$pss] = (int) $row['dataid'];
		
		if(count($datagetquestion[$llii]) > 1){
			$this->kuisoner->deletedataredudan($id_staf,  $datapertanyaaan[$llii], $datagetquestion[$llii][$pss]);
		}
		$pss++;
		}
	}
...
//}
	if($gets == $lastquestion) {
		//$qp=0;
	$qno=0;
	foreach ($this->aspek->get()->result_array() as $row){
		$dataentropyaspek[] = (int) $row['id_aspek'];//aspek 13
		//$data_kategori[] = (string) $row['aspek_subject'];
	}
	for($ty=0; $ty < count($this->aspek->get()->result_array()); $ty++){
	foreach ($this->answer->answerbyiduser($id_staf, $dataentropyaspek[$ty])->result_array() as $row){
		$jum[] = (double) $row['totalpoin'];
	}
}
for($tyq=0; $tyq< count($this->aspek->get()->result_array()); $tyq++){
	foreach ($this->aspek->get($dataentropyaspek[$tyq])->result_array() as $row){
		$max[] = (double) $row['nilai_max_kompeten'];
		$min[] = (double) $row['nilai_min_kompeten'];
	}
}
	for($qp=0; $qp< count($this->aspek->get()->result_array()); $qp++){
	
		//	$entropylog=log($jum, 2);
			//print_r($jum);
			if( $jum[$qp] <  $min[$qp]){
				$prediksi[$qp]="tidak kompeten";
			}
			
			else{
				$prediksi[$qp]="kompeten";
			}

			$param = array(
				'id_user' => $id_staf,
				'nilai_log_entropy' =>$jum[$qp],
				'id_aspek_entropy' => $dataentropyaspek[$qp],
				'prediksi_aspek' => $prediksi[$qp],
	...
	$psl=0;
	for($tyl=0; $tyl< count($this->aspek->get()->result_array()); $tyl++){//aspek 13
	foreach ($this->entropy->cekdatantropy( $id_staf, $dataentropyaspek[$tyl])->result_array() as $row){
		//$dataredudan[$kp] = (int) $row['jumla_redudan'];
	
		$datagetentropyredudan[$tyl][$psl] = (int) $row['id_entropy_del'];//12
	
	if(count($datagetentropyredudan[$tyl]) > 1){
		for($del=0; $del < count($datagetentropyredudan[$tyl])-1; $del++){
		$this->entropy->deletedataredudanentropy($id_staf,  $dataentropyaspek[$tyl], $datagetentropyredudan[$tyl][$psl]);
		}
	}
	$psl++;
...
//print_r(count($datagetentropyredudan[0]));
